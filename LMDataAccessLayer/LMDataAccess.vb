...
#Region "variables"
    ...
    Public Shared _nombreSistema As String
    Public Shared _cadenaConexionReportes As String
...
#Region "Propiedades"
    ...
    Public Shared Property CadenaConexionReportes As String
        Get
            Return _cadenaConexionReportes
        End Get
        Set(value As String)
            _cadenaConexionReportes = value
        End Set
    End Property
...
#Region "Contructores +/Destructor"
    ...
    Public Sub New()
        ...
        If DbSeleccionada IsNot Nothing Then
            ...
            conexion = New SqlConnection(DbSeleccionada)
        Else
...
#Region "Consultas"
    ...
    ''' <remarks></remarks>
    ''' 
    Public Async Function MapearClaseAsync(Of T As New)(RawSql As String, Optional ByVal TipoComando As CommandType = CommandType.Text) As Task(Of List(Of T))

        If Await AbrirConexionAsync() Then
            Try
                comando.CommandText = RawSql
                comando.CommandType = TipoComando
                Dim ReaderAsync As SqlDataReader = Await comando.ExecuteReaderAsync

                Dim Data As List(Of T) = New List(Of T)()
                While (Await ReaderAsync.ReadAsync())
                    Dim NewClass As T = New T()
                    Array.ForEach(Of PropertyInfo)(GetType(T).GetProperties(),
                            Function(Propertie) As PropertyInfo
                                Try
                                    Propertie.SetValue(NewClass, Convert.
                                    ChangeType(ReaderAsync(Propertie.Name), Propertie.PropertyType))
                                Catch ex As Exception
                                End Try
                            End Function)
                    Data.Add(NewClass)
    ...
    Public Function MapearClase(Of T As New)(RawSql As String, Optional ByVal TipoComando As CommandType = CommandType.Text) As List(Of T)
        Dim Data As List(Of T) = New List(Of T)()
        If AbrirConexion() Then
            Try
                comando.CommandText = RawSql
                comando.CommandType = TipoComando
                Dim ReaderAsync As SqlDataReader = comando.ExecuteReader
